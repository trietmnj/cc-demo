...
def extract_mat_struct(filename):
    try:
        ...
        varList = [key for key in list(raw.keys()) if not (key.startswith('__') and key.endswith('__'))]
        mat_struct = {}
        for vv in varList:
            # Define N Columns 
            if raw[vv].ndim == 1:
...
Files_to_load =  ['CHS-NA_nodeID_v4.mat', \
...
SSR_SP = [SRR_LI['SRR']['Col0'][min_indx], \
        SRR_MI['SRR']['Col0'][min_indx], \
        SRR_HI['SRR']['Col0'][min_indx], \
        SRR_All['SRR']['Col0'][min_indx]]*srr_to_stmperyr
...
if 'NACCS' in chs_region:
    ...
    MI_pop = ReducedSet[(ReducedSet['Col6'].values >= 28) & (ReducedSet['Col6'].values < 48)]['Col0'].values
    HI_pop = ReducedSet[ReducedSet['Col6'].values>=48]['Col0'].values
